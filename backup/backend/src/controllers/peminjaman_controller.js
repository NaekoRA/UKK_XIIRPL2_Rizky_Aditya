...
const menyetujuiPeminjaman = (req, res) => {
    ...
    if (req.user.role !== 'petugas') {
        return res.status(403).json({ message: "Akses ditolak. Hanya petugas yang bisa menyetujui peminjaman." });
    }
    ...
    peminjamanModel.menyetujuiPeminjman(id_data_peminjaman, petugasId, status, (err, results) => {
        if (err) return res.status(500).json({ message: "Gagal memproses peminjaman", error: err });
        if (results.affectedRows === 0) return res.status(404).json({ message: "Data peminjaman tidak ditemukan atau sudah disetujui/ditolak" });
